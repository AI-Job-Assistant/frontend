...
export default function Result() {
  ...
  const { faceStats, totalSec, resultSessionId } = useApp(); // 방금 끝낸 면접이면 남아있고, 새로고침/이력조회면 비어있음
  ...
  const [refreshingIdx, setRefreshingIdx] = useState(null); // ← 재분석 중인 문항 index

  const penalty = Number(sessionStorage.getItem("penalty") || 0);
  const extraCount = JSON.parse(sessionStorage.getItem("extraCount") || "[]");

  // ← 추가: context의 faceStats/totalSec가 "지금 보고 있는 이 세션" 것이 맞는지 확인
  //    다른 세션에서 넘어온 잔여값이면 재사용하지 않고 0/null로 취급
  const isOwnResult = !!sessionId && String(resultSessionId) === String(sessionId);
  const displayFaceStats = isOwnResult ? faceStats : null;
  const displayTotalSec = isOwnResult ? totalSec : 0;
    ...
    <Shell>
      ...
      <div style={{ margin: "24px 0 22px" }}>
        ...
        <p style={{ fontSize: 14, color: T.inkSoft, margin: 0 }}>
          ...
          {displayTotalSec > 0 && ` · ${Math.floor(displayTotalSec / 60)}분 ${displayTotalSec % 60}초`}
      ...
      {/* ← 도전 모드는 카메라를 안 쓰므로 Presence 섹션 자체를 숨김 + 다른 세션의 잔여값 사용 안 함 */}
      {displayFaceStats && session?.mode !== "도전" && (
        <Card style={{ padding: "18px 18px 16px", marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 24 }}>
            <div>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: T.ink }}>{displayFaceStats.smiles}회</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: T.inkSoft }}>응시</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: T.ink }}>{displayFaceStats.gazeRate}%</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: T.inkSoft }}>무표정</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: T.ink }}>{displayFaceStats.neutralRate}%</div>
            </div>
          </div>
        </Card>
      )}
